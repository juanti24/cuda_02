...
set_property(TARGET sfml_DEPS_TARGET
             ...
             $<$<CONFIG:Release>:sfml-system;opengl::opengl;sfml-window;Freetype::Freetype;flac::flac;OpenAL::OpenAL;vorbis::vorbis>)
...
########## COMPONENTS TARGET PROPERTIES Release ########################################
    ...
    ########## COMPONENT sfml-main #############
        ...
        set(sfml_sfml-main_FRAMEWORKS_FOUND_RELEASE "")
        conan_find_apple_frameworks(sfml_sfml-main_FRAMEWORKS_FOUND_RELEASE "${sfml_sfml-main_FRAMEWORKS_RELEASE}" "${sfml_sfml-main_FRAMEWORK_DIRS_RELEASE}")
        ...
        set_property(TARGET sfml_sfml-main_DEPS_TARGET
                     APPEND PROPERTY INTERFACE_LINK_LIBRARIES
                     $<$<CONFIG:Release>:${sfml_sfml-main_FRAMEWORKS_FOUND_RELEASE}>
                     $<$<CONFIG:Release>:${sfml_sfml-main_SYSTEM_LIBS_RELEASE}>
                     $<$<CONFIG:Release>:${sfml_sfml-main_DEPENDENCIES_RELEASE}>
                     )

        ####### Find the libraries declared in cpp_info.component["xxx"].libs,
        ####### create an IMPORTED target for each one and link the 'sfml_sfml-main_DEPS_TARGET' to all of them
        conan_package_library_targets("${sfml_sfml-main_LIBS_RELEASE}"
                              "${sfml_sfml-main_LIB_DIRS_RELEASE}"
                              "${sfml_sfml-main_BIN_DIRS_RELEASE}" # package_bindir
                              "${sfml_sfml-main_LIBRARY_TYPE_RELEASE}"
                              "${sfml_sfml-main_IS_HOST_WINDOWS_RELEASE}"
                              sfml_sfml-main_DEPS_TARGET
                              sfml_sfml-main_LIBRARIES_TARGETS
                              "_RELEASE"
                              "sfml_sfml-main"
                              "${sfml_sfml-main_NO_SONAME_MODE_RELEASE}")


        ########## TARGET PROPERTIES #####################################
        set_property(TARGET sfml-main
                     APPEND PROPERTY INTERFACE_LINK_LIBRARIES
                     $<$<CONFIG:Release>:${sfml_sfml-main_OBJECTS_RELEASE}>
                     $<$<CONFIG:Release>:${sfml_sfml-main_LIBRARIES_TARGETS}>
                     )

        if("${sfml_sfml-main_LIBS_RELEASE}" STREQUAL "")
            # If the component is not declaring any "cpp_info.components['foo'].libs" the system, frameworks etc are not
            # linked to the imported targets and we need to do it to the global target
            set_property(TARGET sfml-main
                         APPEND PROPERTY INTERFACE_LINK_LIBRARIES
